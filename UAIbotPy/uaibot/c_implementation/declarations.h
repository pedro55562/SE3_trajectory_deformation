...
struct AABB {
  ...
  AABB();
  static AABB get_aabb_pointcloud(const vector<Vector3f>& points, int start,
                                  int end);
...
struct BVH {
  ...
  static int build_bvh(BVH& bvh, vector<Vector3f>& points, int start, int end,
                       int parentIndex);
  BVH();
...
struct DistStructRobotAuto {
  ...
  DistStructLinkLink get_item(int ind_link_1, int ind_link_2,
                              int ind_obj_link_1, int ind_obj_link_2);
};
...
struct GeometricPrimitives {
 public:
  ...
  static GeometricPrimitives create_box(Matrix4f htm, float width, float depth,
                                        float height);
  static GeometricPrimitives create_cylinder(Matrix4f htm, float radius,
                                             float height);
  static GeometricPrimitives create_pointcloud(vector<Vector3f>& points);
  static GeometricPrimitives create_convexpolytope(Matrix4f htm, MatrixXf A,
                                                   VectorXf b);
  ...
  PrimDistResult dist_to(GeometricPrimitives prim, float h, float eps,
                         float tol, int no_iter_max) const;
  PrimDistResult dist_to(GeometricPrimitives prim, float h, float eps,
                         float tol, int no_iter_max, Vector3f p_A0) const;
...
struct Manipulator {
  ...
  // String
  string toString() const;
  void set_joint_param(int ind_link, float _theta, float _d, float _alpha,
                       float _a, int _joint_type, float _q_min, float _q_max);
  ...
  vector<FKResult> fk(const vector<VectorXf>& q,
                      const vector<Matrix4f>& htm_world_base,
                      bool compute_jac = true) const;

  FKResult fk(VectorXf q, Matrix4f htm_world_base,
              bool compute_jac = true) const;

  vector<FKPrimResult> fk_prim(const vector<VectorXf>& q,
                               const vector<FKResult>& fk_res_all) const;

  FKPrimResult fk_prim(VectorXf q, Matrix4f htm_world_base) const;

  TaskResult fk_task(VectorXf q, Matrix4f htm_world_base,
                     Matrix4f tg_htm) const;

  IKResult ik(Matrix4f tg_htm, Matrix4f htm, VectorXf q0, float p_tol,
              float a_tol, int no_iter_max, bool ignore_orientation) const;

  CheckFreeConfigResult check_free_configuration(
  ...
  DistStructRobotObj compute_dist(GeometricPrimitives obj, VectorXf q,
                                  Matrix4f htm,
                                  DistStructRobotObj old_dist_struct, float tol,
                                  int no_iter_max, float max_dist, float h,
                                  float eps) const;

  DistStructRobotAuto compute_dist_auto(VectorXf q,
                                        DistStructRobotAuto old_dist_struct,
...
PrimDistResult dist_line(vector<Vector3f> a0, vector<Vector3f> a1,
                         vector<Vector3f> b0, vector<Vector3f> b1);

VectorFieldResult vectorfield_rn(VectorXf q, vector<VectorXf>& q_path,
                                 float alpha, float const_velocity,
                                 bool is_closed, float gamma);
...
VectorXf solveQP(const MatrixXf& H, const VectorXf& f, const MatrixXf& A,
                 const VectorXf& b, const MatrixXf& Aeq, const VectorXf& beq);
...
VectorFieldResult vectorfield_SE3(
    ...
    const vector<Eigen::MatrixXd>& curve_derivative =
        std::vector<Eigen::MatrixXd>(),
    double delta = c_delta, double ds = c_ds);
